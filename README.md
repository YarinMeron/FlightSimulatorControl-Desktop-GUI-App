...
On the server we design a generic server that gets a problem and returns a solution(generic server) in this project we implemented a serial server that solves shortest paths problems, The problem is stored in a matrix(Graph) and with Astar and Best First Search algorithms, he returns the shortest path.<br>
